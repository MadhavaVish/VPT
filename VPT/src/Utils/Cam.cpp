...
Cam::Cam(float fov) :m_VerticalFOV(fov)
{
	forwardDir = glm::vec3(0.f, 0.f, -1.f);
	position = glm::vec3(0.f, 0.f, 7.f);
}
bool Cam::OnUpdate(float ts)
{
...
void Cam::OnResize(uint32_t width, uint32_t height)
{
...
const Ray Cam::getPrimaryRay(const int x, const int y, const glm::vec2 offset) const
{
